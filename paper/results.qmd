...
### Decision Tree Multi-actor Hypertension Testing

**Figure 5** presents a classification decision tree predicting the likelihood of Stage 2 hypertension based on multiple individual-level characteristics, including sleep duration, age, stress level, body mass index category, and sleep disorder status. The tree highlights non-linear and interaction effects that are difficult to capture with standard regression models, illustrating how combinations of sleep-related and physiological factors jointly shape hypertension risk.

In particular, the results suggest that sleep disorders and elevated stress levels play a central role in distinguishing high-risk individuals, while sleep duration operates as a secondary but meaningful factor within specific subgroups. This multivariate structure underscores the importance of considering sleep behavior alongside broader health and lifestyle characteristics when assessing cardiovascular risk.
...
This study has several limitations. First, the data are observational, and the results should be interpreted as associations rather than causal effects. Second, sleep duration is self-reported and may be subject to measurement error. Third, the analysis does not control for potential factors such as age, medication use, or current health conditions, which may influence both sleep behavior and blood pressure. Future research could extend this analysis by incorporating additional factors or using longitudinal data to better assess temporal relationships.
